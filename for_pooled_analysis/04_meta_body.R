...
# 04_meta_body.R
#
# Body of the threshold-equivalence analysis, sourced once per outcome by
# 04_meta.R. Do not source this file directly: it expects three objects to be
# set by the driver.
#
#   MX_IN       maxscores rows for ONE outcome (score value x cohort counts)
#   SESP_IN     sesp rows for the SAME outcome (per-site tp/fp/tn/fn)
#   OUT_SUFFIX  filename suffix, "" for the composite and "_<outcome>" otherwise
#
# Nothing else changed from the round-one script. The closing note about a
# future federated extraction for the hospice-excluded subcomposite is now
# obsolete: the site pipeline exports every outcome, and the driver runs them.
#
# Formerly 04_threshold_equivalence.R, which replaced the original 04_meta.R (score x cancer interaction meta-analysis).
...
if (exists("SESP_IN") && nrow(SESP_IN) > 0) {
  
  site_ops = copy(as.data.table(SESP_IN))
  ...
  message("  SESP_IN not available; skipping per-site check")
  site_ops = data.table()
}
...
save_as_docx(
  ...
  path = here("output", "tables", paste0("table_threshold_eq_wide", OUT_SUFFIX, ".docx"))
)
...
save_as_docx(
  ...
  path = here("output", "tables", paste0("table_threshold_eq_sens", OUT_SUFFIX, ".docx"))
)
...
save_as_docx(
  ...
  path = here("output", "tables", paste0("table_threshold_eq_alert", OUT_SUFFIX, ".docx"))
)
